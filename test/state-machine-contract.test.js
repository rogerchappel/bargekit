...
test('events cover speech, barge-in, output control, mute, noise gate, and errors', () => {
  for (const event of ['bargekit.user_speech.started', 'bargekit.barge_in.requested', 'bargekit.output.cancel_requested', 'bargekit.input.noise_gated', 'bargekit.error.recorded']) {
    assert.ok(EVENTS.includes(event));
  }
});

test('configuration fields include timing and barge-in policy', () => {
  assert.deepEqual(REQUIRED_CONFIG_FIELDS, ['mode', 'minSpeechMs', 'silenceMs', 'debounceMs', 'bargeIn.enabled']);
});
